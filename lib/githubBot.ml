...
let github_post repo issue post_body =
  let _, body =
    ...
    and uri =
      Uri.of_string ("https://api.github.com/repos/alan-turing-institute/"
                     ^ repo
                     ^ "/issues/" ^ string_of_int issue ^ "/comments")
    and body = Body.of_string @@ Safe.to_string (`Assoc [("body", `String post_body)])
    in
